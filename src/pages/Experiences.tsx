...
const allExperiences = [
  { icon: <Mountain className="w-8 h-8" />, title: "Trekking & Hiking", count: "20+ Places", desc: "Summit the world's most iconic peaks and trails" },
  { icon: <Palmtree className="w-8 h-8" />, title: "Beach Holidays", count: "10+ Tours", desc: "Crystal waters, white sand, and tropical vibes" },
        ...
        <div className="container">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
            {allExperiences.map((exp, i) => {
              ...
              if (exp.title === "Beach Holidays") {
                return (
                  <Link key={exp.title} to="/packages?category=beach" className="w-full">
                    {content}
                  </Link>
                );
              }
